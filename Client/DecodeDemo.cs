...
namespace DecodeDemo
{
    class Program
    {
        ...
        static List<float> _distance = new List<float>();
        static SportTracksXmlReader.Activity _activity = new SportTracksXmlReader.Activity();
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                var xmlSerializer = new XmlSerializer(typeof(SportTracksXmlReader.Activity));
                _activity.GPSRoute.EncodeBinaryData();
                xmlSerializer.Serialize(new FileStream(@"C:\temp\test.xml", FileMode.CreateNew), _activity);
        ...
        static void DecodeActivitySummary(IEnumerable<Field> summaryFields)
        {
        ...
        static object GetSummaryValue(IEnumerable<Field> summaryFields, string fieldName)
        {
        ...
        private static void WriteFieldWithOverrides(Mesg mesg, byte fieldNumber)
        {
            ...
            foreach (FieldBase field in fields)
            {
                ...
                if (profileField.GetName() == "Distance")
                {
                    _distance.Add((float)field.GetValue());
                }
            }
        }
